...
/// An extension intended for [`EvmEnv`] that adds an extra constructor that creates an instance
/// with [`OpSpecId`].
pub trait ExtOpEvmEnv {
    ...
    /// * `spec` - The chain hardfork description, must implement [`OpHardforks`].
...
impl ExtOpEvmEnv for EvmEnv<OpSpecId> {
    ...
    fn for_op_block(
        ...
        Self::for_block(header, chain_spec, chain_id, blob_params, |c, h| crate::spec(c, h))
    }
}
